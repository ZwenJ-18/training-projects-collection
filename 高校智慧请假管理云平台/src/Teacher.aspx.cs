...
namespace WebLab5
{
    public partial class Teacher : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            // PRG模式：重定向回来后显示审批结果
            if (Session["ApproveMsg"] != null)
            {
                string msg = Session["ApproveMsg"].ToString();
                Session.Remove("ApproveMsg");
                ClientScript.RegisterStartupScript(this.GetType(), "approveAlert",
        ...
        void LoadClassList()
        {
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                string sql = @"
                    ...
                    WHERE u.Role = N'学生'
                      AND c.ClassId = (SELECT ClassId FROM UserInfo WHERE UserName = @TeacherName)
                    ORDER BY u.UserId";
        ...
        protected void gvTeacher_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Approve" || e.CommandName == "Reject")
            {
                ...
                string status = e.CommandName == "Approve" ? "辅导员通过" : "拒绝";
        ...
        void UpdateLeaveStatus(int leaveId, string status)
        {
            ...
            // PRG模式：保存提示消息到Session，重定向避免刷新重复审批
            Session["ApproveMsg"] = status;
