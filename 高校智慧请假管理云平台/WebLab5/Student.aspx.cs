...
namespace WebLab5
{
    public partial class Student : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            // 每次都加载用户信息和请假记录（数据始终最新）
            LoadUserInfo();
            LoadLeaveHistory();

            // 提交成功后的提示（PRG模式：重定向回来显示成功消息）
            if (Session["SubmitSuccess"] != null && Session["SubmitSuccess"].ToString() == "true")
            {
        ...
        void LoadUserInfo()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                if (dr.Read())
                {
                    ...
                    lblCardName.Text = dr["UserName"].ToString();
